...
class Node:
    ...
    def __init__(self, f, parent=None, dims=0, active_dims_idx=None, min_num_variables=3, reset_id=False):
    ...
    def split(self, split_type='median', min_num_variables=3):
        if len(self.active_dims_idx) < min_num_variables:
            return None, None
        ...
        left = Node(self.f, parent=self, dims=self.dims, active_dims_idx=good_idx, reset_id=False)
        left.init_bag(self.features, self.samples, self.feature2sample_map)
        right = Node(self.f, parent=self, dims=self.dims, active_dims_idx=bad_idx, reset_id=False)
    ...
    def get_active_axis_score(self, n=500, sigma=0.01, is_theory=True):
        point = np.random.uniform(self.f.lb, self.f.ub, self.f.dims)
        eps = []
        ...
        F = []
        for i in range(self.dims):
            if i in self.active_dims_idx:
                e = eps[i]
                diff = self.f(np.clip(point + sigma * e, self.f.lb, self.f.ub)) - self.f(np.clip(point - sigma * e, self.f.lb, self.f.ub))
                F.append(diff)
            else:
                F.append(0)
    ...
    def get_axis_score(self, n=500, sigma=0.01, is_theory=True):
        point = np.random.uniform(self.f.lb, self.f.ub, self.f.dims)
        if is_theory:
            eps = []
            for i in range(self.dims):
            ...
            eps = [np.random.randn(self.dims) for _ in range(n)]
        
        F = [self.f(np.clip(point + sigma * e, self.f.lb, self.f.ub)) - self.f(np.clip(point - sigma * e, self.f.lb, self.f.ub)) for e in eps]
        # F = [self.f(np.clip(point + sigma * e, self.f.lb, self.f.ub)) for e in eps]
        grad = np.asarray([Fi*epsi for Fi, epsi in zip(F, eps)])
        grad = grad.sum(axis=0) / (2 * n * sigma**2)
        axis_score = np.abs(grad)
