...
from LamctsVS_theory.Node import Node
...
class MCTS:
    ...
    def populate_training_data(self):
        self.nodes.clear()
        self.ROOT = Node(self.func, parent=None, dims=self.dims, active_dims_idx=list(range(self.dims)), reset_id=True)
...
#                 break
    ...
    def search(self, max_samples, verbose=True):
        ...
        while True:
            ...
            left_kid, right_kid = leaf.split(self.split_type, self.min_num_variables)
            if left_kid is not None and right_kid is not None:
                self.nodes.append(left_kid)
                self.nodes.append(right_kid)
